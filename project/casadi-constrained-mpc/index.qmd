...
The two obstacles are now defined through center-position parameters. That makes it easy to rerun the same controller with different obstacle layouts while keeping the model, cost, and constraints unchanged.

![Scenario geometry with parameterized obstacles and sampled bicycle states](assets/plots/mpc-geometry.svg)

The geometry figure shows the lane, road corridor, two elliptical obstacle regions, and sampled bicycle footprints along the closed-loop trajectory. The vehicle trajectory bends around the obstacle ellipses while staying within the road corridor. That is the visible consequence of the optimization problem.

## The Prediction Model

The controller uses a **kinematic bicycle model**, a common reduced-order model for path tracking. If you want the model from first principles before reading the controller details, start with the standalone explanation here: [Bicycle Model from First Principles](../../blog/bicycle-model-basics/).
...
![Closed-loop path and velocity profile](assets/plots/mpc-summary.svg)

The top panel shows the closed-loop path and obstacle constraints. The bottom panel shows the velocity profile against the position-dependent speed reference. The speed dips near the obstacle-rich parts of the road, which gives the controller more room to satisfy the steering and obstacle constraints smoothly.
...
![Animated closed-loop vehicle motion](assets/plots/mpc-animation.gif)

The animation uses the same closed-loop states as the static figures. The red bicycle footprint advances one MPC sample at a time, so the avoidance maneuver is easier to read as a receding-horizon response instead of a single finished curve.

## Four Obstacle Layouts

Because the obstacle centers are parameters, the same problem can be repeated with different geometry:

1. Lower then upper obstacles: `O1=(17.0, -1.5)`, `O2=(34.0, 2.3)`.
2. Middle gate: `O1=(20.0, -1.8)`, `O2=(35.0, 1.8)`.
3. Late gate: `O1=(23.0, -1.4)`, `O2=(39.0, 1.4)`.
4. Same-side lower obstacles: `O1=(20.0, -1.6)`, `O2=(36.0, -0.4)`.

![Closed-loop paths for four obstacle-position scenarios](assets/plots/mpc-scenario-comparison.svg)

The different obstacle positions change where the controller starts steering away from the centerline and how aggressively it returns to the reference lane.

![Acceleration and steering profiles for four obstacle-position scenarios](assets/plots/mpc-control-comparison.svg)

The control comparison is the most direct way to see the practical effect of geometry. A tighter or more central obstacle pair usually creates larger steering activity, while offset obstacles can often be handled with a gentler lateral shift.

![Metric comparison for four obstacle-position scenarios](assets/plots/mpc-metrics-comparison.svg)

The metric comparison summarizes three differences: the closest normalized obstacle clearance, speed-tracking error, and total steering variation. Those numbers are also saved in the scenario comparison CSV for further analysis.
...
- [Scenario comparison metrics CSV](assets/code/scenario_comparison_metrics.csv)

To regenerate the figures from this project folder:

```bash
python project/casadi-constrained-mpc/assets/code/mpc_bicycle_constraints.py
```

That generates the figures used on this page and saves them under `project/casadi-constrained-mpc/assets/plots/`.
